package com.haierbiomedical.ttslibrary.util;
...
public class AutoCheck {
    ...
    public void check(final InitConfig initConfig) {
        Thread t = new Thread(new Runnable() {
            @Override
            public void run() {
                AutoCheck autoCheck = innerCheck(initConfig);
                isFinished = true;
                String message = autoCheck.obtainDebugMessage();
                Log.d("AutoCheck>>>>>>",message);
            }
        });
        t.start();
